...
# importa funções que medem o tamanho REAL dos PNGs em memória
from .utils import (
...
def plot_comparison(result):
    """
    Plota comparação lado-a-lado usando TAMANHOS REAIS de PNG.
    ...
    # Tamanhos REAIS em PNG (em memória), sem optimize
    png_o_mb = compute_png_size_rgb_mb(orig, optimize=False)
    png_c_mb = compute_png_size_palette_mb(result, optimize=False)

    ratio = png_o_mb / png_c_mb if png_c_mb > 0 else np.inf
    reduction = (1 - png_c_mb / png_o_mb) * 100 if png_o_mb > 0 else 0.0

    fig, ax = plt.subplots(1, 2, figsize=(16, 8))

    ax[0].imshow(orig)
    ax[0].set_title(
        f'Original\n{png_o_mb:.2f} MB PNG | {colors_o:,} cores',
        fontsize=14
    )
    ax[0].axis('off')
    ...
    ax[1].set_title(
        f'Comprimida (K={K})\n'
        f'{png_c_mb:.2f} MB PNG | {colors_c:,} cores',
        fontsize=14
    )
    ax[1].axis('off')

    plt.suptitle(
        f'PNG real: {ratio:.2f}x menor '
        f'({reduction:.1f}% redução) | PSNR: {psnr:.1f} dB',
        fontsize=16, y=0.95
    )
    ...
    print(
        f"\n📦 Tamanho real em PNG:"
        f"\n   Original : {png_o_mb:.3f} MB"
        f"\n   K-Means  : {png_c_mb:.3f} MB (K={K})"
        f"\n   Fator    : {ratio:.2f}x | Redução: {reduction:.1f}%\n"
    )
...
def plot_result(result, show_comparison=True, show_colors=False,
    ...
    Args:
        ...
        show_comparison: Comparação lado-a-lado (com PNG real)
